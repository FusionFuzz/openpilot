...
class FCWChecker():
  def __init__(self):
    ...
    # addition
    self.pm = messaging.PubMaster(['fcwData'])
    self.last_fcw_time_no_reset = 0
    self.fcw_total_times = 0
  ...
  def update(self, mpc_solution, cur_time, active, v_ego, a_ego, x_lead, v_lead, a_lead, y_lead, vlat_lead, fcw_lead, blinkers):
    ...
    if (fcw_lead > 0.99):
      ...
      # addition
      dat = messaging.new_message('fcwData')
      dat.fcwData = {
          "ttc": ttc,
          "ttcCounter": self.counters['ttc'],

          "vLeadMax": self.v_lead_max,
          "vLeadMaxCounter": self.counters['v_lead_max'],

          "vEgo": v_ego,
          "vLead": v_lead,
          "vEgoLeadCounter": self.counters['v_ego_lead'],

          "leadSeen": self.counters["lead_seen"],

          "yLead": y_lead,
          "yLeadCounter": self.counters['y_lead'],

          "vLatLead": vlat_lead,
          "vLatLeadCounter": self.counters['vlat_lead'],

          "aThr": a_thr,
          "aDelta": a_delta,
          "lastMinA": self.last_min_a,
          ...
          "blinkersCounter": float(self.common_counters['blinkers']),

          "vEgoCounter": int(self.common_counters['v_ego']),

          "fcwLead": int(fcw_lead),
          "futureFcw": int(future_fcw),

          "lastFcwTime": self.last_fcw_time,
          "curTime": cur_time,

          "lastFcwTimeNoReset": self.last_fcw_time_no_reset,
          "fcwTotalTimes": self.fcw_total_times,

      }
      self.pm.send('fcwData', dat)
      ...
      if future_fcw and (self.last_fcw_time + 5.0 < cur_time):
        ...
        # addition
        self.last_fcw_time_no_reset = cur_time
        self.fcw_total_times += 1
        # modification
        activate_fcw = True
