...
class Controls:
  def __init__(self, sm=None, pm=None, can_sock=None):
    ...
    if self.sm is None:
      ...
      # self.sm = messaging.SubMaster(['deviceState', 'pandaState', 'modelV2', 'liveCalibration',
      #                                'driverMonitoringState', 'longitudinalPlan', 'lateralPlan', 'liveLocationKalman',
      #                                'managerState', 'liveParameters', 'radarState', 'parametersFromCarla'] + self.camera_packets,
      #                                ignore_alive=ignore, ignore_avg_freq=['radarState', 'longitudinalPlan'])
      # not used modification sync:
    ...
    self.rk = Ratekeeper(1/DT_CTRL, print_delay_threshold=None)
    # freq modification sync:
  ...
  def update_events(self, CS):
    ...
    # modification: 65->85
    if self.sm['deviceState'].memoryUsagePercent > (90 if TICI else 85):
    ...
    if len(self.sm['radarState'].radarErrors):
      ...
      # debug addition sync:
      self.sm.all_alive_and_valid(verbose=True)
      # debug addition sync:
  ...
  def data_sample(self):
    ...
    self.sm.update(0)
    # if CS.canValid:
    #     while not self.sm.all_alive_and_valid():
    #         self.sm.update(100)
    # start modification sync:
  ...
  def state_transition(self, CS):
    ...
    # self.v_cruise_kph = int(self.sm['parametersFromCarla'].targetSpeed)
    # not used modification sync:
