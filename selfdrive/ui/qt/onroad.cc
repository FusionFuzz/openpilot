...
void OnroadAlerts::updateState(const UIState &s) {
  ...
  if (sm["deviceState"].getDeviceState().getStarted()) {
    ...
    // unused debug modification sync:
    } else if ((sm.frame - s.scene.started_frame) > 20 * UI_FREQ) {
    // } else if ((sm.frame - s.scene.started_frame) > 20 * UI_FREQ) {

      // std::cout << std::endl << std::endl << "DEBUG: accessfile() called by process " << ::getpid() << " (parent: " << ::getppid() << ")" << std::endl << std::endl << std::endl;
      // std::cout << std::endl << std::endl << "sm.frame " << sm.frame << " s.scene.started_frame " << s.scene.started_frame << std::endl << std::endl << std::endl;
      // unused debug modification sync:
