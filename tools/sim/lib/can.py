...
# modification
def can_function(pm, speed, angle, idx, cruise_button, is_engaged, radar_data=[]):
  ...
  # modification
  # Note: in current radard implementation, "NEW_TRACK": 0 or 1 will influence VleadK, aleadK, and aLeadTau but these variables are not actively used by fcw but used by longitudinal control
  from common.realtime import frequency
  from math import floor
  step_size = int(floor(0.05 / frequency))
  if idx % step_size == 0:
    msg.append(rpacker.make_can_msg("RADAR_DIAGNOSTIC", 1, {"RADAR_STATE": 0x79}, -1))
    for i in range(16):
      if len(radar_data) == 16:
        msg.append(rpacker.make_can_msg("TRACK_%d" % i, 1, {"LONG_DIST": radar_data[i][1], 'LAT_DIST': radar_data[i][2], "NEW_TRACK": 1, 'REL_SPEED': radar_data[i][0]}, -1))
      else:
        msg.append(rpacker.make_can_msg("TRACK_%d" % i, 1, {"LONG_DIST": 255.5}, -1))
